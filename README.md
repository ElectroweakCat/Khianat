# Online chess engine Khianat
Play against the chess engine online on https://khianat.org/

## About
Khianat is a very simple chess engine/AI in JavaScript. 

The primary goal of chess engine is the decision-making (AI) part of the application. All functionality outside the scope of the AI are implemented using external libraries:
- Chessboard GUI: Using the chessboard.js API (Chris Oakman (https://chessboardjs.com/index.html)
- Game Mechanics: Using the chess.js API (Jeff Hlywa (https://github.com/jhlywa/chess.js)

## How to Play?
1. Head over to link

2. Play as white by dragging a piece to your desired location. The AI plays as black.

3. Click .. to start

## How does it work?
1. Head over to link

2. Play as white by dragging a piece to your desired location. The AI plays as black.

3. Click .. to start

## Note
This is my first larger JavaScript project so the code might lack of efficiency and proper notation. Furthermore, the website rn..

## License
Use of this project is governed by the [MIT License](LICENSE).
